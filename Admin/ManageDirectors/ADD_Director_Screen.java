...
public class ADD_Director_Screen extends clsScreen {
    ...
    public static void ShowAddDirectorScreen(String ID) {
        clsScreen.DrawScreenHeader("ADD Director SCREEN", null);

        if (ID == null) {
            System.out.print("Enter ID of the New Director: ");
        ...
        switch(result){
            case SVSUCCEDED:
                if(clsAdminLog.InsertingAdminLog(enAction.ADD_DIRECTOR, enTargetModel.DIRECTOR, NewDirector.get_ID(), Description)){
                    _PrintData(NewDirector);    
                    System.out.printf("Director (%s) Has Been Created Successfully :)\n",NewDirector.get_ID());
                    System.out.println("THE LAST OPERATION HAS BEEN STORED SUCCESSFULLY!");
                    
                }else{
                    NewDirector.DELETED();
                    System.out.printf("Director (%s) has not been added to the system!",NewDirector.get_ID());
            ...
            case SvFailedObjectAlreadyExists:
                System.out.println("SAVE FAILED Director USERNAME ALREADY EXISTS!");
            case SvFailedInvalidInsertation:
                    System.out.println("SAVE FAILED Insertation Process Was Invalid!");
                break;            
        }
    }
