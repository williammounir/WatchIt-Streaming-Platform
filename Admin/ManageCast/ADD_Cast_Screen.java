...
public class ADD_Cast_Screen extends clsScreen {
       ...
       //            String socialMediaLink) {
        ...
        do{
            ...
            if(!clsMovie.IsMovieExists(MID)){
                System.out.printf("Movie With ID: %s Doesn't Exists in the System, Do you Want to add the information about this Movie and then Continoue With the process of adding the cast?\n",MID);
    ...
    public static void ShowAddCastScreen() {
        clsScreen.DrawScreenHeader("ADD CAST SCREEN", null);
        System.out.print("Enter ID of the New Cast: ");
        ...
        switch(result){
            case SVSUCCEDED:
                if(clsAdminLog.InsertingAdminLog(enAction.ADD_CAST, enTargetModel.CAST, NewCast.get_ID(), Description)){
                    _PrintData(NewCast);    
                    System.out.printf("Cast (%s) Has Been Created Successfully :)\n",NewCast.get_ID());
                    System.out.println("THE LAST OPERATION HAS BEEN STORED SUCCESSFULLY!");
                    
                }else{
                    NewCast.DELETEC();
                    System.out.printf("Cast (%s) has not been added to the system!",NewCast.get_ID());
